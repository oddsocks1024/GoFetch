...
-> Go Fetch by Ian Chapman (c) 1999
-> Version 1.2
...
CONST RELEASE=5, HYPHEN=45, FSLASH=47, SPACE=32, TR_ADDREQUEST=9
...
->IF (localebase:=OpenLibrary('locale.library',38))=NIL THEN Raise(ERR_NOLOC)
->catalog:=OpenCatalogA(NIL,'gofetch.catalog',[OC_BUILTINLANGUAGE,'english',TAG_DONE])
...
menu := [ NM_TITLE,0, 'Project'  , 0 ,0,0,0,
          ...
          NM_ITEM ,0, 'Keyfile Info',0,0,0,ID_KEYINFO,
...
app:=ApplicationObject,
    ...
    MUIA_Application_Version    , '$VER: Go Fetch! 1.2',
    MUIA_Application_Copyright  , '© 1999 Ian Chapman written for Zeus Developments',
    ...
    SubWindow, mui_mainwindow:=WindowObject,
        MUIA_Window_Title       , 'Go Fetch! 1.2 by Ian Chapman',
        MUIA_Window_ID          , "GOFE",
        ...
        WindowContents, VGroup,
            Child, RegisterGroup(mui_pages),
                Child, VGroup,
                    Child, TextObject,
                        TextFrame,
                        MUIA_Text_Contents, '\n\ecGo Fetch! a specialised FTP/HTTP Client\n ©1999 Ian Chapman.\nWritten for Zeus Developments\n',
                ...
                Child, VGroup,
                    ...
                    Child, TextObject,
                            TextFrame,
                            MUIA_Text_Contents, '\ec\nGo Fetch! Preferences\n',
                            MUIA_ShortHelp, '\ecGo Fetch! Preferences',
                            MUIA_Background, MUII_FILLBACK,
                            End, ->TextObject
    ...
    SubWindow, mui_addwindow:= WindowObject,
        ...
        WindowContents, VGroup,
            ...
            Child, VGroup,
                ...
                Child, mui_nagtext:=TextObject,
                        TextFrame,
                        MUIA_Text_Contents, '\ecHTTP Requires a Keyfile\nSee Documentation on Registering',
                        MUIA_ShortHelp,'HTTP is for registered users only.',
                        End, -> TextObject
...
set(mui_nagtext, MUIA_ShowMe, FALSE)
setupprofiles()
...
result:= doMethodA(app, [MUIM_Application_Input,{signal}])
    ...
    SELECT result
        ...
        CASE ID_ABOUT -> About menu option
            Mui_RequestA(app, mui_mainwindow, 0, 'About Go Fetch!','*_OK','\ecGo Fetch! FTP/HTTP © 1999 Ian Chapman. Written for Zeus Developments\nVersion 1.2 (RELEASE 6)\n\nGo Fetch! is a passive FTP/HTTP Client, designed to download a queue\nof files in the background automatically\n\nE-Mail: ian.chapman@bleach.demon.co.uk\n\nThanks to:\nMarc Bradshaw & Contributors for AREXX Scripts\nNeil Williams for Beta Testing\nStefan Stunz for MUI\n\nAREXX PORT: GOFETCH',NIL)
        ...
        CASE ID_ADD -> Add button on main window
            ...
            IF registeredflag=FALSE
                set(mui_nagtext, MUIA_ShowMe, MUI_TRUE)
...
EXCEPT DO
    ...
    END st
    IF registeredflag=FALSE
        IF app THEN Mui_RequestA(app, mui_mainwindow, 0, 'Nag Requester (QUITTING)', '*_OK','\ecYou are using an \ebUnregistered\en version of Go Fetch!\n\nIf you would like to register,\nsee the documenation for details.\n\nFor any registration queries please contact:\nian.chapman@bleach.demon.co.uk',NIL)
    ENDIF
...
SELECT exception
    ...
    CASE ERR_NOREQ
         EasyRequestArgs(NIL,[SIZEOF easystruct,
                                NIL,
                                'GoFetch! Error',
                                'Unable to open reqtools.library V38+',
                                'OK',
                                TAG_DONE],
                                NIL, NIL)
    ...
    CASE ERR_NOAPP
             EasyRequestArgs(NIL,[SIZEOF easystruct,
                                NIL,
                                'GoFetch! Error',
                                'Unable to create the Go Fetch! Application\nThe possible causes are:\nGo Fetch! is already running\nYou are low on memory (512K Required)',
                                'OK',
                                TAG_DONE],
                                NIL, NIL)
        ->WriteF('Unable to create application\nPossible Causes:\n   Go Fetch! FTP/HTTP is already running\n   You are low on memory\n')
    CASE ERR_NOICON
         EasyRequestArgs(NIL,[SIZEOF easystruct,
                                NIL,
                                'GoFetch! Error',
                                'Unable to open icon.library V33+',
                                'OK',
                                TAG_DONE],
                                NIL, NIL)
    ...
    CASE ERR_NOLOC
         EasyRequestArgs(NIL,[SIZEOF easystruct,
                                NIL,
                                'GoFetch! Error',
                                'Unable to open locale.library V40+',
                                'OK',
                                TAG_DONE],
                                NIL, NIL)
        ->WriteF('Unable to open locale.library V40+\n')
    CASE ERR_NOBSD
         EasyRequestArgs(NIL,[SIZEOF easystruct,
                                NIL,
                                'GoFetch! Error',
                                'Unable to open bsdsocket.library\nPlease run your TCP/IP Stack',
                                'OK',
                                TAG_DONE],
                                NIL, NIL)
    ...
    CASE ERR_NOGAD
         EasyRequestArgs(NIL,[SIZEOF easystruct,
                                NIL,
                                'GoFetch! Error',
                                'Unable to open gadtools.library V39+',
                                'OK',
                                TAG_DONE],
                                NIL, NIL)
...
ELSE
    ...
    ELSE
        ...
        IF registeredflag=TRUE
            ...
            Close(fh)
        ELSE
            StrCopy(httpsite,'UNREGISTERED')
            StrCopy(httpport,'0')
            StrCopy(httplocalpath,'UNREGISTERED')
        ENDIF
    ENDIF
        ...
        WHILE err<>0
            ...
            IF StrCmp(teststr,'*',1)=TRUE
                IF registeredflag=TRUE
                    ...
                    doMethodA(mui_profilelist, [MUIM_List_InsertSingle, addtolist, MUIV_List_Insert_Bottom ])
                ELSE
                    doMethodA(mui_profilelist, [MUIM_List_InsertSingle, 'HTTP Profile Detected. To use HTTP you must register!', MUIV_List_Insert_Bottom ])
                ENDIF
            ENDIF
...
IF registeredflag=TRUE THEN status:=1
...
StrCopy(key_string,'\ecGo Fetch! is UNREGISTERED!\n\nPlease Consider Registering!')

IF (keyfh:=Open('PROGDIR:gofetch.key',MODE_OLDFILE))<>NIL
    ...
    IF checksum=checktest
        registeredflag:=TRUE
        StringF(key_string,'\ecName: \s\nAddress: \s\nKeyfile Number: \s\n\nThanks for your support!',name,address,keynum)
    ELSE
        StrCopy(key_string,'\ecWarning: You have an invalid Keyfile\nPlease contact the author.')
    ENDIF
